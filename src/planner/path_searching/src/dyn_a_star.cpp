...
{
    // ROS_ERROR("start_pt x=%.2f, y=%.2f, z=%.2f",start_pt[0],start_pt[1],start_pt[2]);
    Eigen::Vector3d tmp;
    if (!Coord2Index(start_pt, start_idx) || !Coord2Index(end_pt, end_idx))
    {
        return false;
    }
    // ROS_ERROR("----------wjf-----11111111111------------");
    ...
    if (checkOccupancy(Index2Coord(start_idx)))
    {
        ...
        do
        {
            tmp = (start_pt - end_pt).normalized();
            tmp[2] = 0;
            start_pt =  tmp * step_size_ + start_pt;
            // start_pt = (start_pt - end_pt).normalized() * step_size_ + start_pt;
            if (!Coord2Index(start_pt, start_idx))
            {
                return false;
            }
            // ROS_ERROR("----------wjf-----2222222222222222------------");
        } while (checkOccupancy(Index2Coord(start_idx)));
    }
    // ROS_ERROR("start_idx x=%d, y=%d, z=%d",start_idx[0],start_idx[1],start_idx[2]);
    ...
    if (checkOccupancy(Index2Coord(end_idx)))
    {
        ...
        do
        {
            tmp = (end_pt - start_pt).normalized();
            tmp[2] = 0;
            end_pt =  tmp * step_size_ + end_pt;
            // end_pt = (end_pt - start_pt).normalized() * step_size_ + end_pt;
            if (!Coord2Index(end_pt, end_idx))
            {
                return false;
            }
            // ROS_ERROR("----------wjf-----3333333333333------------");
        } while (checkOccupancy(Index2Coord(end_idx)));
    }

    // ROS_ERROR("end_pt x=%.2f, y=%.2f, z=%.2f",end_pt[0],end_pt[1],end_pt[2]);
    return true;
...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check)
{
    ...
    {
        // ROS_ERROR("Unable to handle the initial or end point, force return! test-----");
        return false;
    }
    ...
    openSet_.push(startPtr); //put 333333333333start in open set
    ...
    {
        // ROS_ERROR("openset_size:%d",openSet_.size());
        // ROS_ERROR("----------wjf-----8------------");
        num_iter++;
        // ROS_ERROR("----------wjf-----9------------");
...
vector<Vector3d> AStar::astarSearchAndGetSimplePath(const double step_size, Vector3d start_pt, Vector3d end_pt){
    ...
    {
        // ROS_ERROR("----------------wjf:14");
        Vector3d pt_tmp(0.0,0.0,-999.0);
        path.push_back(pt_tmp);
        return path;
