...
private:
	...
	{
		// ROS_ERROR("checkOccupancy pos x=%.2f, y=%.2f, z=%.2f",pos[0],pos[1],pos[2]);
		// ROS_ERROR("return bool is %d", (bool)grid_map_->getInflateOccupancy(pos));
...
inline Eigen::Vector3d AStar::Index2Coord(const Eigen::Vector3i &index) const
{
	...
	// ROS_ERROR("return Vector3d x=%.2f, y=%.2f, z=%.2f",start_idx[0],start_idx[1],start_idx[2]);
};
...
inline bool AStar::Coord2Index(const Eigen::Vector3d &pt, Eigen::Vector3i &idx) const
{
	...
	ROS_WARN("positon x=%.2f, y=%.2f, z=%.2f, index=%d %d %d", pt[0], pt[1], pt[2], idx(0), idx(1), idx(2));
	if (idx(0) < 0 || idx(0) >= POOL_SIZE_(0) || idx(1) < 0 || idx(1) >= POOL_SIZE_(1) || idx(2) < 0 || idx(2) >= POOL_SIZE_(2))
	{
		ROS_ERROR("positon x=%.2f, y=%.2f, z=%.2f",pt[0],pt[1],pt[2]);
